...
const detectImage = async (
  ...
  for (let idx = 0; idx < selected.dims[1]; idx++) {
    ...
    const box = data.slice(0, 4) as any;
    const scores = data.slice(4) as any; // classes probability scores
    const score = Math.max(...(scores as any)); // maximum probability scores
    const label = scores.indexOf(score as any); // class id of maximum probability scores
